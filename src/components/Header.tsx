...
import { selectAccessToken, useGetUserInfoQuery } from '../redux/reducers/userSlice'
...
const ProfileButtons = () => {
    ...
    const {data: userInfo, isLoading, error} = useGetUserInfoQuery({accessToken: accessToken!}, {skip: accessToken === undefined})
    if (accessToken) {
        return (
            <ButtonWrapper>
                <Link to={ '/liked' }>
                ...
                <Link to={ '/profile' }>
                    <Avatar sx={ { width: 60, height: 60 } } src={ userInfo?.picture } alt={ 'profilePic' }/>
                </Link>
            </ButtonWrapper>
        )
    } else {
