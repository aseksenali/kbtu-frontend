...
const HeartIcon: React.FC<HeartProps> = (props) => {
    const toggleLiked = (evt: React.MouseEvent<SVGSVGElement>) => {
        evt.stopPropagation()
        props.toggleLiked && props.toggleLiked();
    }

    return (
        <HeartWrapper width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onClick = {toggleLiked}>
            <path fill={props.liked ? '#ec3c3c' : 'none'}
                  ...
                  stroke={props.liked ? 'none' : "#465564"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        </HeartWrapper>
    )
}
